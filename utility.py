...
class ArrayManager(object):
    ...
    def sma(self, n: int, array: bool = False) -> Union[float, np.ndarray]:
        ...
        if array:
            return result
        return result[-1]
    def sma_volume(self, n: int, array: bool = False) -> Union[float, np.ndarray]:
        """
        Simple moving average of volume.
        """
        result: np.ndarray = talib.SMA(self.volume, n)
        if array:
            return result
        return result[-1]
    ...
    def vwap(self, n: int) -> float:
        ...
        """
        closes = self.close[-n:]
        volumes = self.volume[-n:]
        if np.sum(volumes) == 0:
            return 0
        return np.sum(closes * volumes) / np.sum(volumes)

    def trend_signal_offset(self, N: int = 15, window_size: int = 3750, beta_window: int = min(7500, len(self.close))) -> float:
        """
        计算趋势信号偏移量，返回当前时刻的 β*M_t
        N: 收益率计算步长
        window_size: M_t 的滑动窗口
        beta_window: β 的计算窗口
        """
        import numpy as np
        ...
        if len(close) < N + 1:
            return 0.0
        R_a = np.log(close[-1] / close[-N-1])

        # 2. 计算滑动均值 M_t
        if len(close) < window_size + N:
            M_t = np.mean([np.log(close[i+N] / close[i]) for i in range(len(close)-N)])
        else:
            M_t = np.mean([np.log(close[i+N] / close[i]) for i in range(-window_size-N, -N)])

        # 3. 计算 β 系数
        if len(close) < beta_window + N:
            beta = 0.9
        else:
            R_a_series = np.array([np.log(close[i+N] / close[i]) for i in range(-beta_window-N, -N)])
            M_t_series = np.convolve(R_a_series, np.ones(window_size)/window_size, mode='valid')
            if len(M_t_series) == 0:
                beta = 0.9
            else:
                M_R_product = np.mean(M_t_series * R_a_series[-len(M_t_series):])
                M_squared = np.mean(M_t_series ** 2)
